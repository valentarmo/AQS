        ...
        stage('Deploy Buckets') {
            steps {
                withEnv(['PATH+EXTRA=/usr/local/bin']) {
                    ...
                    sh "python3 scripts/deploy-buckets.py --StackName ${env.AQS_STACK_NAME} --S3GlueBucketName ${env.AQS_S3_GLUE_BUCKET_NAME} --S3RawDataBucketName ${env.AQS_S3_RAW_DATA_BUCKET_NAME} --S3ProcessedDataBucketName ${env.AQS_S3_PROCESSED_DATA_BUCKET_NAME} --S3AthenaBucketName ${env.AQS_S3_ATHENA_BUCKET_NAME} --S3GlueScriptsBucketName ${env.AQS_S3_GLUE_SCRIPTS_BUCKET_NAME} --S3TaskcatBucketName ${evn.AQS_S3_TASKCAT_BUCKET_NAME}"
